    ...
    , ui(new Ui::MainWindow)
{
    ...
    LoadOptions();
    toggleOnDuplicateProcess = QApplication::arguments().count() > 1
                                   ? QApplication::arguments().at(1) == "--toggleOnDup"
                                   : false;
...
void MainWindow::OpenDiaglog(bool isChecked)
{
    ...
    if (isChecked) {
        QString AlarmLocation = QFileDialog::getOpenFileName(this,
                                                             "Select Alarm File",
                                                             QDir::homePath());
...
void MainWindow::PMWarning()
{
    QMessageBox::warning(this,
                         "Time Verification",
                         "Your selected time is set for PM. If you intended to wake in the "
                         "morning, now is your chance to fix it");
}
...
void MainWindow::SetupSlots(QAction *QAquit, QAction *QAshow)
{
    ...
    connect(trayIcon,
            ...
            this,
            SLOT(ToggleWindow(QSystemTrayIcon::ActivationReason)));
    ...
    connect(ui->listAlmBtn,
            SIGNAL(clicked(QAbstractButton *)),
            this,
            SLOT(AddRemoveAlarm(QAbstractButton *)));
...
void MainWindow::on_btnCountdown_clicked() {
    CountdownTimer dialog(this);

    // 1. Otevři dialog a počkej na potvrzení
    if (dialog.exec() == QDialog::Accepted) {
        int seconds = dialog.getTotalSeconds();
        if (seconds <= 0) return; // Nic nedělej, pokud je čas 0

        // 2. Vytvoř nový "One-Shot" budík
        ScheduleModel *newSche = new ScheduleModel();

        // Nastav čas na "teď + zadané sekundy"
        QDateTime target = QDateTime::currentDateTime().addSecs(seconds);
        newSche->AlarmTime = target.time();
        newSche->CustomAlarm = target.date();

        // 3. Využij to, co už v kódu máš
        newSche->isOneshot = true;
        newSche->isCustomAlarmEnabled = true; // Aby bral CustomAlarm jako platný

        // 4. Přidej do tvého stávajícího systému
        _Schedules->AddSchedule(newSche);
        _Schedules->Save(); // Ulož si to

        // 5. Obnov seznam v GUI
        PopulateListWidget();

        qInfo() << "One-shot budík přidán na:" << target.toString();
    }
}